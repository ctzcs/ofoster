...
// ===== merged from Graphics/SpriteFont.odin =====
...
SpriteFontMake :: proc(font:^Font,size:f32=16,codepoints:[]int=nil)->SpriteFont {
	...
	result.Image=new(Image); result.Image^=ImageMake(2048,2048); result.OwnsImage=true
	points:=codepoints;if len(points)==0 { ascii:=SpriteFontAscii(); points=ascii[:] }
	x,y,row:=0,0,0
	for cp in points {
		ch:=FontGetCharacter(font,cp,scale); c:=SpriteFontCharacter{Codepoint=cp,Advance=ch.Advance,Offset=ch.Offset,Exists=true}
			...
			if y+ch.Height<result.Image.Height {
				...
				for py in 0..<ch.Height {
					for px in 0..<ch.Width {
						ImageSetPixel(result.Image,x+px,y+py,Color{255,255,255,bmp[py*ch.Width+px]})
					}
				}
				c.Subtexture=Subtexture{Source=Rect{f32(x),f32(y),f32(ch.Width),f32(ch.Height)},Frame=Rect{-ch.Offset[0],-ch.Offset[1],f32(ch.Width),f32(ch.Height)}}
...
SpriteFontMakeGPU :: proc(device:^GraphicsDevice, font:^Font, size:f32=16, codepoints:[]int=nil, premultiply_alpha:bool=true)->SpriteFont {
	...
	if result.Image != nil && result.OwnsImage { ImageClear(result.Image); free(result.Image); result.Image=nil; result.OwnsImage=false }
	packer := PackerMake()
	packer.MaxSize = 8192
	packer.Padding = 1
	packer.Trim = true
	...
	for i := 0; i < len(result.Characters); i += 1 {
		...
		glyph := FontGetImage(font, FontGetCharacter(font, ch.Codepoint, FontGetScale(font, size)))
		if glyph.Width <= 0 || glyph.Height <= 0 do continue
		idx := PackerAdd(&packer, "", glyph)
		append(&packer_indices, i)
		_ = idx
	}
	out := PackerPack(&packer)
	for page in out.Pages {
		page_value := page
		if premultiply_alpha do ImagePremultiply(&page_value)
		tex := new(Texture)
	...
	for entry in out.Entries {
		...
		source := Rect{f32(entry.Source.X), f32(entry.Source.Y), f32(entry.Source.Width), f32(entry.Source.Height)}
		frame := Rect{f32(entry.Frame.X), f32(entry.Frame.Y), f32(entry.Frame.Width), f32(entry.Frame.Height)}
		old.Subtexture = SubtextureMake(result.GeneratedTextures[entry.Page], source, frame)
		result.Characters[character_index] = old
	}
	if len(result.GeneratedTextures) > 0 { result.Texture = result.GeneratedTextures[0] }
	return result
}
SpriteFontFromMsdf :: proc(device:^GraphicsDevice,msdf:^MsdfFont)->SpriteFont{result:=SpriteFont{GraphicsDevice=device};if msdf==nil{return result};result.Size=msdf.Size;result.Ascent=msdf.Ascent;result.Descent=msdf.Descent;result.LineGap=msdf.LineGap;result.LineHeight=msdf.LineHeight;result.Image=&msdf.Image;result.Sampler=TextureSamplerMake(TextureFilter.Linear,TextureWrap.Clamp);append(&result.NewlineCharacters,'\n');append(&result.WordbreakCharacters,'\n',' ');for k in msdf.Kerning{append(&result.Kerning,SpriteFontKerning{k.First,k.Second,k.Advance})};if device!=nil{tex:=TextureFromImage(device,&msdf.Image,"MsdfFont");result.Texture=new(Texture);result.Texture^=tex;append(&result.GeneratedTextures,result.Texture);if device.Defaults.Initialized{result.Material=MaterialClone(&device.Defaults.MsdfMaterial);distance:=[1]f32{msdf.DistanceRange};uniform:[]u8=make([]u8,size_of(distance));mem.copy(raw_data(uniform),raw_data(distance[:]),size_of(distance));MaterialStageSetUniformBuffer(&result.Material.Fragment,uniform,0);result.HasMaterial=true}};for ch in msdf.Characters{source:=ch.SourceRect;frame:=Rect{-ch.Offset[0],-ch.Offset[1],source.Width,source.Height};sub:=SubtextureMake(result.Texture,source,frame);append(&result.Characters,SpriteFontCharacter{ch.Codepoint,sub,ch.Advance,ch.Offset,true})};return result}
...
SpriteFontBindTexture :: proc(font:^SpriteFont,texture:^Texture){if font==nil{return};for i:=0;i<len(font.Characters);i+=1{c:=font.Characters[i];if c.Subtexture.Source.Width>0&&c.Subtexture.Source.Height>0{c.Subtexture=SubtextureMake(texture,c.Subtexture.Source,c.Subtexture.Frame);font.Characters[i]=c}}}
...
SpriteFontDispose :: proc(font:^SpriteFont){if font==nil{return};for tex in font.GeneratedTextures{if tex!=nil{TextureDispose(tex);free(tex)}};clear(&font.GeneratedTextures);font.Texture=nil;if font.Image!=nil&&font.OwnsImage{ImageClear(font.Image);free(font.Image)};font.Image=nil;clear(&font.Characters);clear(&font.Kerning);clear(&font.NewlineCharacters);clear(&font.WordbreakCharacters);font.HasMaterial=false}
SpriteFontGetKerning :: proc(font:^SpriteFont,a,b:int,size:f32=0)->f32{if font==nil{return 0};s:=size;if s==0{s=font.Size};for k in font.Kerning{if k.First==a&&k.Second==b{if font.Size>0{return k.Advance*(s/font.Size)};return k.Advance}};if font.KerningFont==nil{return 0};scale:=FontGetScale(font.KerningFont,s);return FontGetKerning(font.KerningFont,a,b,scale)}
...
SpriteFontSizeOf :: proc(font:^SpriteFont,text:string,size:f32=0)->Vec2{return Vec2{SpriteFontWidthOf(font,text,size),SpriteFontHeightOf(font,text,size)}}
...
sprite_font_draw_impl :: proc(batch:^Batcher,font:^SpriteFont,text:string,position,justify:Vec2,size:f32,color:Color){if batch==nil||font==nil{return};scale:=f32(1);if font.Size>0{scale=size/font.Size};BatcherPushMatrix2D(batch,position,Vec2{scale,scale},0,true);if font.HasMaterial{BatcherPushMaterial(batch,&font.Material)};BatcherPushSampler(batch,font.Sampler);prev_texture:=batch.Texture;at:=Vec2{0,font.Ascent};just_scale:=f32(1);if font.Size>0&&size>0{just_scale=font.Size/size};if justify[0]!=0{at[0]-=justify[0]*SpriteFontWidthOfLine(font,text,size)*just_scale};if justify[1]!=0{at[1]-=justify[1]*SpriteFontHeightOf(font,text,size)*just_scale};last:=0;for r in text{if sprite_font_is_newline(font,r){at[0]=0;at[1]+=font.LineHeight;last=0;continue};cp:=int(r);if c,ok:=SpriteFontFindCharacter(font,cp);ok{if last!=0{at[0]+=SpriteFontGetKerning(font,last,cp,size)};if c.Subtexture.Texture!=nil{BatcherImage(batch,c.Subtexture,Vec2{at[0]+c.Offset[0],at[1]+c.Offset[1]},color)};at[0]+=c.Advance};last=cp};batch.Texture=prev_texture;BatcherPopSampler(batch);if font.HasMaterial{BatcherPopMaterial(batch)};BatcherPopMatrixStack(batch)}
sprite_font_draw_simple :: proc(batch:^Batcher,font:^SpriteFont,text:string,position:Vec2,color:Color){sprite_font_draw_impl(batch,font,text,position,{},font.Size,color)}
sprite_font_draw_sized :: proc(batch:^Batcher,font:^SpriteFont,text:string,position:Vec2,size:f32,color:Color){sprite_font_draw_impl(batch,font,text,position,{},size,color)}
sprite_font_draw_justified :: proc(batch:^Batcher,font:^SpriteFont,text:string,position,justify:Vec2,color:Color){sprite_font_draw_impl(batch,font,text,position,justify,font.Size,color)}
sprite_font_draw_justified_sized :: proc(batch:^Batcher,font:^SpriteFont,text:string,position,justify:Vec2,size:f32,color:Color){sprite_font_draw_impl(batch,font,text,position,justify,size,color)}
...
sprite_font_draw_wrapped_simple :: proc(batch:^Batcher,font:^SpriteFont,text:string,position:Vec2,max_width:f32,color:Color){if font==nil{return};line:=0;for range in SpriteFontWrapText(font,text,max_width,font.Size){line_text:=text[range.Start:range.Start+range.Length];SpriteFontDraw(batch,font,line_text,Vec2{position[0],position[1]+f32(line)*font.LineHeight},color);line+=1}}
sprite_font_draw_wrapped_justified :: proc(batch:^Batcher,font:^SpriteFont,text:string,position,justify:Vec2,max_width:f32,color:Color){if font==nil{return};line:=0;for range in SpriteFontWrapText(font,text,max_width,font.Size){line_text:=text[range.Start:range.Start+range.Length];SpriteFontDraw(batch,font,line_text,Vec2{position[0],position[1]+f32(line)*font.LineHeight},justify,color);line+=1}}
sprite_font_draw_wrapped_sized :: proc(batch:^Batcher,font:^SpriteFont,text:string,position:Vec2,max_width,size:f32,color:Color){if font==nil{return};line:=0;for range in SpriteFontWrapText(font,text,max_width,size){line_text:=text[range.Start:range.Start+range.Length];SpriteFontDraw(batch,font,line_text,Vec2{position[0],position[1]+f32(line)*font.LineHeight},size,color);line+=1}}
sprite_font_draw_wrapped_justified_sized :: proc(batch:^Batcher,font:^SpriteFont,text:string,position,justify:Vec2,max_width,size:f32,color:Color){if font==nil{return};line:=0;for range in SpriteFontWrapText(font,text,max_width,size){line_text:=text[range.Start:range.Start+range.Length];SpriteFontDraw(batch,font,line_text,Vec2{position[0],position[1]+f32(line)*font.LineHeight},justify,size,color);line+=1}}
